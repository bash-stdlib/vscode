...
suite("HTML Parser Test Suite", () => {
  ...
  suite("when parsing HTML with escaped entities", () => {
    suite("in function names", () => {
      let functions: ShdocFunction[];

      setup(() => {
        const html = `<section id="parametrize">
          <h3>&#64;parametrize<a class="headerlink"></a></h3>
          <p>Test function.</p>
        </section>`;
      ...
      test("should decode numeric entities", () => {
        assert.strictEqual(functions[0].name, "@parametrize");
      });
    });

    suite("in descriptions", () => {
      let functions: ShdocFunction[];

      setup(() => {
        const htmlWithNumericEntities = `<section id="test">
          <h3>test_func<a class="headerlink"></a></h3>
          <p>Uses &#64;fixture and &#64;vary prefixes.</p>
        </section>`;

        functions = parser.parse(htmlWithNumericEntities, {
          isTesting: true,
        });
      });
      ...
      test("should decode numeric entities", () => {
        assert.strictEqual(
          ...
          "Uses @fixture and @vary prefixes.",
        );
      });
    });

    suite("in descriptions with named entities", () => {
      let functions: ShdocFunction[];

      setup(() => {
        const html = `<section id="test">
          <h3>test_func<a class="headerlink"></a></h3>
          <p>Test &amp; verify &lt;tag&gt; and "quoted" &amp; 'apostrophe'.</p>
        </section>`;
      ...
      test("should decode named entities", () => {
        assert.strictEqual(
          functions[0].description,
          "Test & verify <tag> and \"quoted\" & 'apostrophe'.",
        );
      });
    });

    suite("in descriptions with unrecognized entities", () => {
      let functions: ShdocFunction[];

      setup(() => {
        const html = `<section id="test">
          <h3>test_func<a class="headerlink"></a></h3>
          <p>Unknown entity &#999; remains.</p>
        </section>`;

        functions = parser.parse(html, { isTesting: true });
      });

      test("should preserve unrecognized entities", () => {
        assert.ok(
          functions[0].description.includes("&#999;") ||
            functions[0].description.includes("Unknown"),
        );
      });
    });
    ...
    suite("in arguments", () => {
      ...
      setup(() => {
        const html = `<section id="test">
          ...
          <p>Test.</p>
          <section id="id1">
            <h4>Arguments<a class="headerlink"></a></h4>
            <ul>
              <li><p><strong>$1</strong> (string): Prefix &#64;fixture for setup.</p></li>
            </ul>
          </section>
        </section>`;
      ...
      test("should decode numeric entities", () => {
        assert.strictEqual(
          functions[0].args[0].desc,
          "Prefix @fixture for setup.",
        );
      });
    });
  });
