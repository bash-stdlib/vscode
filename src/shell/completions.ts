...
export function createCompletionItem(parsedFunction: ShdocFunction): vscode.CompletionItem {
  ...
  completionItem.insertText = getSnippetOrName(parsedFunction);
...
function getSnippetOrName(parsedFunction: ShdocFunction): vscode.SnippetString | string {
  if (parsedFunction.args && parsedFunction.args.length > 0) {
    const snippetArguments = parsedFunction.args
      ...
      .join(" ");
    return new vscode.SnippetString(`${parsedFunction.name} ${snippetArguments}`);
  }
